...
public class CreateProjectUndoHandler implements UndoHandler {

    private final ComponentRepository componentRepo;

    @Override
    public boolean supports(String commandType) {
        return "CREATE_PROJECT".equals(commandType);
    }

    @Override
    public CommandResult<?> undo(CommandLog log, String userName) {
        componentRepo.deleteById(log.getEntityId());
        return new CommandResult<>(userName, "component", log.getEntityId(), "UNDO_CREATE_PROJECT",
                null, null, null);
    }
}

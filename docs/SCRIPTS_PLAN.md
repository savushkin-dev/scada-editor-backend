...
> Снимок на 22.07.2026. С 23.07.2026 обработчики событий живут в отдельной таблице
> `editor.component_event` (в дереве — `events[]` компонента), а ключ `events` в `image`
> вычищается при сохранении — см. «Переезд событий» ниже.
...
| Реакция на клик оператора | **фронт → бэк** | `events[]` (`event_type: onClick`); если пишет тег — делегирует бэку через `ACTION` | клик |
...
## Переезд событий из `image` в `component_event` (23.07.2026)

Обработчики событий больше не хранятся в `component_state.image.events`. Причина: `image`
принадлежит **состоянию**, а обработчик — **компоненту**; в старой схеме `onClick` пришлось
бы дублировать в каждое состояние, а перерисовка фигуры во фронте затирала бы логику вместе
с графикой.

| Что | Где |
|---|---|
| Таблица | `editor.component_event (id, component_id, event_type, script)`, `UNIQUE (component_id, event_type)`, CHECK на `event_type` |
| Сущность/DTO | `ComponentEvent`, `EventPayloadDto`/`EventResponseDto`, `EventMapper` |
| В дереве проекта | `events[]` на уровне узла, рядом со `scripts[]` и `bindings[]` |
| Синхронизация при сейве | `ComponentScriptBindingApplier` (clear + addAll, как у scripts/bindings) |
| Защита `image` | `ComponentServiceImpl.stripEvents` вырезает ключ `events` при сохранении |

Типы: `onClick`, `onDoubleClick`, `onMouseDown`, `onMouseUp`, `onHover`, `onHoverOut`,
`onOpen`, `onClose`. Все исполняются **на фронте** — событие порождает жест конкретного
оператора. Колонки «где исполнять» нет: серверная логика запускается по `ACTION` (`scripts`)
или по смене тега (`on_change`), а клиентский обработчик делегирует бэку через `runScript`.
Периодические серверные задачи (watchdog в ПЛК) сюда не относятся — им нужна своя сущность
уровня проекта, не зависящая от вёрстки мнемосхемы.

Мигрировать было нечего: событий в `image` на момент переезда не существовало.

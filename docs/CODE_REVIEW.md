...
**Прогресс разбора (обновлено 2026-07-27):** ✅ **разобраны все пункты** (#1–#13,
включая оба подпункта #12 и JWT-секрет #13f). Под каждым заголовком — блок-цитата с
сутью правки; каждая группа собрана и закоммичена отдельно. Действие для фронта:
WebSocket подключать как `/ws/runtime/{sessionId}?token=<jwt>` (см. #12).
...
> **Исправлено 2026-07-27:** все шесть подпунктов (✅).
...
- ✅ **JWT-секрет с fallback в репозитории.** `gateway`, `runtime` и прочие
  ...
  → `JwtSecretGuard` в `auth`/`gateway`/`runtime` на старте: при активном профиле
  `prod` со встроенным дефолтом сервис не стартует (fail-fast), вне prod — WARN в лог.
  Стенд не тронут. Мёртвый `jwt.secret` из `channel`/`editor` (не использовался) убран.

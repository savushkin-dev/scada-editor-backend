...
| Runtime рвёт WebSocket с 401 | WS теперь идёт **через gateway**: `ws://localhost:8080/ws/runtime/{sessionId}?token=<jwt>`. Токен проверяет gateway, runtime ждёт от него заголовок `X-Username`. Прямое подключение на `:8085` в обход gateway отдаёт 401 — для локального прогона без gateway поднять runtime с `RUNTIME_WS_REQUIRE_AUTH=false` |

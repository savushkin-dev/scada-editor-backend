...
**2. Исполнение `onClick` (фронт).** `runEvent` берёт `el.events.onClick`, собирает скоуп
...
читает `{idNode, value}`, кладёт в своё состояние, **сразу публикует новое значение** в
`scada.tags` (ключ = `idNode`, тело `{value}`) и результат в `scada-command-results`. Это
...
- **Нет `onClick` у кнопки** → клик не долетает до `runScript` (в `image` состояния должно
  быть `events.onClick`). Резолв скрипта — по **имени**, поэтому стоек к смене id.
